...
document.getElementById("agencia").addEventListener("change", (e) => {
    fetch('api_ejecutivos', {
        ...
        if (data.data.length > 0) {
            ...
            let radio = document.querySelectorAll('input[type=radio][name=ejecutivo_id]')
            checkedRadio(radio);
        }
    }).catch(err => console.error(err));
});
...
document.getElementById("btn_agregar").addEventListener("click", (e) => {
    ...
    fetch('api_ejecutivos_tem', {
        ...
        body: JSON.stringify({
            ...
            ejecutivo_id: parseInt(document.querySelector("input[name='ejecutivo_id']").value),
...
function limpiar() {
    ...
    document.querySelector('input[type=radio][name=ejecutivo_id]').checked = false;
}
